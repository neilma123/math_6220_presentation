...
## Figure Placeholder: Gradient Noise Is Not Gaussian

**Paper Figure 1 placeholder**

Insert the three-panel histogram comparison:

- real gradient-noise norm from AlexNet on CIFAR10;
- scaled Gaussian norm;
- scaled $\alpha$-stable norm.

Main message:

> The real histogram visually resembles the heavy-tailed stable sample more than the Gaussian one.

Suggested presentation use: spend about 45 seconds here, because this figure motivates the whole change of model.
...
| Special case | baseline | $\alpha=2$ gives scaled Brownian motion |

The crucial behavioral change is not just "more noise."

It is discontinuity: large jumps make barrier height less controlling.

## Figure Placeholder: Stable Densities and Levy Paths

**Paper Figure 2 placeholder**

Insert the two-panel figure:

- left: $S\alpha S$ densities for $\alpha=1.8$ and $\alpha=2$;
- right: paths of $L_t^\alpha$ for $\alpha=1.8$ and $\alpha=2$.

Main message:

> As soon as $\alpha<2$, the tails get heavier and paths can jump.

Suggested explanation: Brownian motion "wiggles"; Levy motion sometimes "teleports."
...
Assume $f$ has local minima

$$
m_1,\dots,m_r
$$

separated by local maxima
...
## Figure Placeholder: Two Valleys and Tail Estimator

**Paper Figure 3 placeholder**

Insert the two-panel figure:

- left: objective with two local minima and a separating maximum;
- right: validation of the tail-index estimator $\widehat{\alpha}$.

Suggested use:

- left panel goes with the stationary probability formula;
- right panel transitions into the empirical methodology.
...
Stable laws are stable under sums:

$$
X_1+\cdots+X_{K_1}
\sim
S\alpha S(K_1^{1/\alpha}\sigma).
$$
...
**Paper Figures 4 and 5 placeholders**

Insert:

- Figure 4: $\widehat{\alpha}$ for FCNs as width and depth vary;
- Figure 5: CNN accuracy and $\widehat{\alpha}$ as width varies.

Main findings:

- estimated $\alpha$ is far below $2$ in all shown configurations;
- for MNIST FCNs, larger networks often produce smaller $\alpha$;
- for CIFAR10 FCNs, $\alpha$ is roughly stable around $1.1$-$1.2$;
- for CNNs, under-parametrized models have especially low $\alpha$, then stabilize near $1.0$-$1.1$.

Takeaway:

> Gradient noise is consistently heavy-tailed, and the tail index depends on architecture and data.

## Results: Minibatch Size

**Paper Figure 6 placeholder**

Insert the minibatch-size plots for FCNs.

The common intuition is:

> Larger minibatches should make gradient noise more Gaussian.

But the paper finds:

- increasing minibatch size has little effect on $\widehat{\alpha}$;
- even large minibatches remain far from $\alpha=2$;
- training and test accuracies are comparable across these minibatch sizes.

Interpretation:

The usual CLT story may be missing the infinite-variance or heavy-tail structure of the summands.

## Results: Evolution During Training

**Paper Figure 7 placeholder**

Insert the iteration-wise plots of:
...
L_t^\alpha = \sqrt{2}B_t.
$$

So Brownian motion is not discarded; it is the endpoint of a broader stable-family model.
